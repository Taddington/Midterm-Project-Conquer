...
public class UpdateUI : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!isPaused)
        {
            ...
            if ((CrossPlatformInputManager.GetButtonDown("Pause") || Input.GetKeyDown(KeyCode.Escape) || Input.GetKeyDown(KeyCode.P)) && !statScreen.activeSelf && SceneManager.sceneCount == 1)
                PauseGame();

            if (CrossPlatformInputManager.GetButtonDown("Open Stats"))
            {
                OpenStats();
            }

            if (CrossPlatformInputManager.GetButtonDown("Open Shop"))
                OpenShop();
            #endregion
        }
    }
